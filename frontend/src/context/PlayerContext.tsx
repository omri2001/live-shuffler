...
import { fetchCurrentTrack, fetchQueue, queueSkip, addSourceWithProgress, syncCurrentTrack } from '../api/spotify';
  ...
  switch (action.type) {
    case 'SET_TRACK': {
      ...
      if (queueIdx >= 0 && state.queue[queueIdx]._scores) {
        effectiveTrack = { ...action.track, _scores: state.queue[queueIdx]._scores };
      } else if (state.currentTrack?._scores && state.currentTrack.id === action.track.id) {
    ...
    case 'SET_QUEUE': {
      ...
      if (state.currentTrack && queueCurrent && state.currentTrack.id !== queueCurrent.id) {
        const idx = newQueue.findIndex((t: Track) => t.id === state.currentTrack!.id);
        if (idx >= 0) {
          newIndex = idx;
          newCurrentTrack = { ...state.currentTrack, _scores: newQueue[idx]._scores };
        } else {
...
export function PlayerProvider({ children, authenticated }: { children: ReactNode; authenticated: boolean }) {
      ...
      try {
        ...
        if (playback?.item) {
          ...
          if (queueResult && queueResult.sources.length > 0) {
            ...
            if (playingId !== queueCurrentId && playingId !== lastSyncedIdRef.current) {
  ...
  return (
    <PlayerContext.Provider value={{ state, dispatch, refreshQueue, startupDone, startupMessage }}>
      {children}
