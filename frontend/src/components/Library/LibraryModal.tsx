...
function sourceLabel(key: string, playlists: Playlist[], albums: Album[]): { name: string; detail: string; img?: string } {
  if (key === 'liked') {
  ...
  if (key.startsWith('playlist:')) {
    ...
    return { name: pl?.name ?? 'Playlist', detail: `${pl?.tracks.total ?? '?'} tracks`, img: pl?.images?.[0]?.url };
  }
  if (key.startsWith('album:')) {
    const id = key.slice(6);
    const al = albums.find(a => a.id === id);
    return { name: al?.name ?? 'Album', detail: al?.artists.map(a => a.name).join(', ') ?? '', img: al?.images?.[0]?.url };
  }
...
export default function LibraryModal({ open, onClose, onError }: LibraryModalProps) {
  ...
  useEffect(() => {
    ...
    if (tab === 'playlists' && playlists.length === 0) {
      setLoading(true);
      api.fetchPlaylists().then(d => setPlaylists(d.items || [])).finally(() => setLoading(false));
    }
    if (tab === 'albums' && albums.length === 0) {
      setLoading(true);
      api.fetchAlbums().then(d => setAlbums((d.items || []).map(i => i.album))).finally(() => setLoading(false));
    }
  }, [open, tab]);

  const handleAdd = async (source: string, playlistId?: string, albumId?: string) => {
    const key = playlistId ? `playlist:${playlistId}` : albumId ? `album:${albumId}` : source;
          ...
          {tabs.map(t => (
            <button
              ...
              className={`px-4 py-2.5 text-sm font-medium transition-colors relative ${
                tab === t.key ? 'text-spotify-white' : 'text-spotify-gray hover:text-spotify-white'
              }`}
            >
              {t.label}
              {tab === t.key && (
          ...
          <div className="px-6 py-3 border-b border-spotify-dark-lighter">
            ...
            <div className="w-full h-1.5 rounded-full bg-spotify-dark-lighter overflow-hidden">
              <div
                ...
                style={{ width: addingTotal > 0 ? `${Math.round((addingProgress / addingTotal) * 100)}%` : '0%' }}
              />
            </div>
          </div>
        )}
          ...
          {tab === 'active' && (
            state.sources.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8">
                <p className="text-spotify-gray text-sm">No sources added yet</p>
                <p className="text-spotify-gray text-xs mt-1">Add songs from the other tabs</p>
              </div>
            ) : (
                    ...
                    <div key={key} className="flex items-center justify-between p-3 rounded-lg bg-spotify-dark-lighter/50">
                      <div className="flex items-center gap-3 min-w-0">
                        {key === 'liked' ? (
                          ...
                          <img src={info.img} alt="" className="w-10 h-10 rounded object-cover shrink-0" />
                        ) : (
                          <div className="w-10 h-10 rounded bg-spotify-dark-lighter shrink-0" />
                        )}
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-spotify-white truncate">{info.name}</p>
                          <p className="text-xs text-spotify-gray truncate">{info.detail}</p>
                        </div>
                      </div>
                      <button
            ...
            loading ? (
              ...
              <div className="flex flex-col gap-2">
                {playlists.map(pl => {
                  ...
                  return (
                    <div key={pl.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-spotify-dark-lighter/50 transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        {img ? (
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-spotify-white truncate">{pl.name}</p>
                      ...
                      {loadedSources.has(key) ? (
                        ...
                        >
                          {adding === key ? addingStatus || 'Adding...' : 'Add'}
                        </button>
                      )}
                    </div>
                  );
            ...
            loading ? (
              ...
              <div className="flex flex-col gap-2">
                {albums.map(al => {
                  ...
                  return (
                    <div key={al.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-spotify-dark-lighter/50 transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        {img ? (
                          <img src={img} alt="" className="w-12 h-12 rounded object-cover" />
                        ) : (
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-spotify-white truncate">{al.name}</p>
