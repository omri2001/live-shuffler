...
export default function GenreCircles({
  ...
  const handleRandomMetrics = () => {
    const newValues = Object.fromEntries(
      metricNames.map((m) => [
        m,
        Math.random() < 0.4 ? 0 : Math.round(Math.random() * 100),
      ]),
    );
  ...
  const handleCircleClick = (name: string) => {
    if (soloMetric === name) {
      const restored =
        savedValues.current ??
        Object.fromEntries(allMetricNames.map((m) => [m, 0]));
      setValues(restored);
      ...
      const soloValues = Object.fromEntries(
        allMetricNames.map((m) => [m, m === name ? 100 : 0]),
      );
      ...
      if (hovering) {
        ...
        if (el) {
          ...
          if (container) {
            ...
            if (
              circleCenter < containerRect.left ||
              circleCenter > containerRect.right
            ) {
              return;
            }
          }
  ...
  const handleWheel = useCallback(
    (name: string, e: React.WheelEvent) => {
      e.preventDefault();
      const raw = -e.deltaY;
      const delta =
        Math.sign(raw) * Math.max(1, Math.round(Math.abs(raw) / 20));
      setValues((prev) => {
  ...
  const renderCircle = (name: string) => (
    ...
    >
      <MetricCircle
        ...
        circleRef={(el) => {
          if (el) circleRefs.current.set(name, el);
        }}
      />
    </div>
  );
      ...
      <div className="pointer-events-auto relative w-full max-w-3xl px-12 my-auto">
        ...
        {layout === "carousel" && (
          <>
            ...
            <div
              className="absolute left-12 top-0 bottom-0 w-12 z-[5] pointer-events-none"
              style={{
                background:
                  "linear-gradient(to right, var(--color-spotify-dark), transparent)",
              }}
            />
            <div
              className="absolute right-12 top-0 bottom-0 w-12 z-[5] pointer-events-none"
              style={{
                background:
                  "linear-gradient(to left, var(--color-spotify-dark), transparent)",
              }}
            />
            <div
              ref={scrollRef}
        ...
        {layout === "grid" && (
          <div
            className="grid justify-center gap-4 px-4 py-4"
            style={{
                    ...
                    {allMetricNames.map((name) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {selected && (
                            <span className="ml-auto text-spotify-green text-xs">
                              &#10003;
                            </span>
                          )}
                        </button>
