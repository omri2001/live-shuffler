  ...
  const metrics = stats
    ? Object.entries(stats.metrics)
        .map(([name, buckets]) => ({
          name,
          buckets,
          nonZero: BUCKET_ORDER.slice(1).reduce(
      ...
      <div className="relative bg-spotify-dark-light rounded-xl shadow-2xl w-[680px] max-w-[90vw] max-h-[80vh] border border-spotify-dark-lighter flex flex-col">
        ...
        <div className="px-6 py-5 overflow-y-auto">
          {!stats || stats.total === 0 ? (
            ...
            <>
              <p className="text-xs text-spotify-gray mb-5">
                {stats.total} unplayed tracks
              </p>
              <div className="grid grid-cols-2 gap-x-6 gap-y-5">
                {metrics.map(({ name, buckets }) => {
                  const maxCount = Math.max(
                    ...BUCKET_ORDER.map((b) => buckets[b] ?? 0),
                    1,
                  );
                    ...
                    <div key={name}>
                      ...
                      <div className="flex gap-1 mt-1">
                        {BUCKET_ORDER.map((label) => (
                          <span
                            key={label}
                            className="flex-1 text-center text-[9px] text-spotify-gray"
                          >
                            {label}
                          </span>
                        ))}
