...
export default function SuggestionsModal({ open, onClose }: SuggestionsModalProps) {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(() => {
      api.suggestionsList().then(d => setSuggestions(d.suggestions)).catch(() => {});
    }, 5000);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-spotify-dark-light rounded-xl shadow-2xl w-[520px] max-w-[90vw] max-h-[80vh] border border-spotify-dark-lighter flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-spotify-dark-lighter shrink-0">
          <h2 className="text-lg font-bold text-spotify-white">Song Requests</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 px-6">
              <p className="text-spotify-gray text-sm mb-2">Requests are disabled</p>
              <p className="text-spotify-gray text-xs text-center">Enable song requests in Settings</p>
            </div>
          ) : (
            <div className="p-6">
              {/* QR + link */}
              <div className="flex items-center gap-5 mb-6 p-4 rounded-lg bg-spotify-dark-lighter/50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-spotify-gray mb-2">Share this link or QR code</p>
                  <div className="flex gap-2">
                    <code className="flex-1 text-xs text-spotify-white bg-spotify-dark rounded px-2 py-1.5 truncate">{suggestUrl}</code>
                    <button onClick={handleCopy} className="px-3 py-1.5 rounded bg-spotify-green text-black text-xs font-semibold hover:bg-spotify-green-hover transition-colors shrink-0">
                      {copied ? 'Copied!' : 'Copy'}
                    </button>
                  </div>
                </div>
              ...
              {suggestions.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-spotify-gray text-xs mt-1">Share the QR code and wait for suggestions</p>
                </div>
              ) : (
                <div className="space-y-2">
                  <p className="text-xs text-spotify-gray mb-2">{suggestions.length} request{suggestions.length !== 1 ? 's' : ''}</p>
                  {suggestions.map(s => (
                    <div key={s.track_id} className="flex items-center gap-3 p-3 rounded-lg bg-spotify-dark-lighter/30 hover:bg-spotify-dark-lighter/50 transition-colors">
                      {s.track.image ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-spotify-white truncate">{s.track.name}</p>
                      ...
                      <span className="text-[10px] text-spotify-gray shrink-0 w-12 text-right">{timeAgo(s.first_requested)}</span>
