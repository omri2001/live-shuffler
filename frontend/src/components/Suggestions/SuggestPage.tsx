...
export default function SuggestPage({ code }: SuggestPageProps) {
  ...
  const [submitted, setSubmitted] = useState<Set<string>>(() => {
    try {
      return new Set(
        JSON.parse(localStorage.getItem(`suggest_votes_${code}`) || "[]"),
      );
    } catch {
      return new Set();
    }
  ...
  useEffect(() => {
    const fetch = () =>
      api
        .suggestionsPool(code)
        .then((d) => {
          setPool(d.suggestions);
          const currentIds = new Set(
            d.suggestions.map((s: SuggestionItem) => s.track_id),
          );
          setSubmitted((prev) => {
  ...
  const handleSuggest = async (track: SuggestionTrack) => {
    if (submitted.has(track.id) && pool.some((s) => s.track_id === track.id))
      return;
    try {
    ...
    return (
      <div className="min-h-screen bg-spotify-dark flex items-center justify-center p-6">
        <div className="text-center">
          <p className="text-spotify-white text-lg font-bold mb-2">
            Session Closed
          </p>
          <p className="text-spotify-gray text-sm">
            The DJ has stopped accepting requests
          </p>
        </div>
      </div>
    );
        ...
        {query.length >= 2 && (
          <div className="px-4 py-3">
            {searching ? (
              ...
              <p className="text-spotify-gray text-sm text-center py-4">
                No results
              </p>
            ) : (
              <div className="space-y-2">
                {results.map((track) => {
                  ...
                  const justSubmitted =
                    submitted.has(track.id) && alreadyInPool;
                  return (
                    <div
                      key={track.id}
                      className="flex items-center gap-3 p-3 rounded-xl bg-spotify-dark-light"
                    >
                      {track.image ? (
                        <img
                          src={track.image}
          ...
          <div className="px-4 py-3">
            ...
            <div className="space-y-2">
              {pool.map((s) => (
                <div
                  key={s.track_id}
                  className="flex items-center gap-3 p-3 rounded-xl bg-spotify-dark-light/50"
                >
                  {s.track.image ? (
                    <img
                      src={s.track.image}
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {s.count > 1 && (
                      <span className="px-2 py-0.5 rounded-full bg-spotify-green/20 text-spotify-green text-xs font-bold">
                        {s.count}
                      </span>
                    )}
                    {submitted.has(s.track_id) ? (
                      ...
                      <button
                        onClick={() =>
                          handleSuggest(s.track as SuggestionTrack)
                        }
        ...
        {pool.length === 0 && query.length < 2 && (
          <div className="flex flex-col items-center justify-center py-16 px-6">
            <p className="text-spotify-gray text-sm text-center">
              Search for a song to suggest it to the DJ
            </p>
          </div>
        )}
      </div>
